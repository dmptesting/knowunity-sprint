...
**statTile** — component set, one variant property: `stat` (`xp` / `score` / `blazing`). No other exposed properties. Layers per variant: `Label` (small chip) above `Timer` (icon group + value text).

> A compact tile that surfaces a single stat (XP earned, accuracy score, streak). Reach for it inline after a recall attempt or on a summary screen when one number deserves emphasis. Don't use it for multi-line content or anything longer than a short label + short value; it isn't a card.

Each variant carries its own accent, and `stat` is the only thing that changes it: `xp` → `accent/blue`, `score` → `accent/green`, `blazing` → `accent/brand`. Within a variant the tile's fill and its 2px stroke are that accent's `bold`, the `Label` text is the matching `onBold` (Body S Bold), and the inner `Timer` panel is `background/page` with its icon and value both in the accent's `bold`. Structure is a vertical auto-layout frame: a hugging `Label` chip centred on the top 20px, then `Timer` filling the width below it — the dark panel sits over the coloured tile so only a coloured strip shows behind the label, so `Timer` must stay the second child or it covers the label.

Built by converting three real instances already in the file in place into the `stat=xp` / `stat=score` / `stat=blazing` variants, so every colour here is the real thing, not reverse-engineered. The radii are bound (`Radius/400` on the tile and `Timer`, `Radius/100` on `Label`), and the value carries the text style `Greed Condensed/Stat` — a new style, added because the design drew a family and a size the scale didn't have. It snaps to `font/size/lg` (21) and `font/lineHeight/sm` (20); Figma drew 24, which is not a step on the scale.

Where the React build departs: the labels are sentence case per this repo's rule ("XP" / "Score" / "Time") against Figma's uppercase, and `blazing` names the variant and its accent rather than the label — "BLAZING" over an elapsed time would praise a slow run as readily as a fast one. **No webfont ships for `Greed Condensed-TRIAL`**, so `font/family/default` leads the CSS fallback chain and the numerals render in Greed Standard at the same weight and size until a condensed face is added to `public/fonts/`. The icon snaps to `Icon/300` (24) from Figma's 29, the tile hugs rather than taking Figma's untokenised 108.25px width, and Figma's 2px stroke is dropped in code because it uses the same `bold` token as the fill and so paints nothing.
...
**bottomSheet** — component set, variant property `result`: `correct` / `incorrect`. Combined from two loose frames on 2026-09-13. The result sheet (`app/_screens/resultSheet/`) composes it for every verdict in the recall loop.
...
**verdictHeader** — the word that opens a result screen, variant `verdict`: `pass` / `partial` / `fail`. Only `pass` gets a coloured treatment; `partial` and `fail` are neutral and differ only in copy, per SPEC.md. **Unused in the app** since the Result screen was removed (2026-09-15) — verdicts show in bottomSheet. Don't reach for it on the result sheet.
